...
def _get_stats(tweet, clean_tweet):
    ...
    user_profile_location = tweet['user']['location']

    stats_record = {'tweet_id': tweet_id, 'raw_tweet': raw_tweet, 'norm_tweet': norm_tweet,
...
def build_data(gran, minsubset, maxsubset):
    ...
    if gran in {"states", "cities"}:
                            ...
                            if tweet['place']['place_type'] == 'city':
                                ...
                                if len(dataset[key]) <= maxsubset:
                                    ...
                                    if clean_tweet:
                                        ...
                                        subsets_stats[key].append(_get_stats(
                                            tweet, clean_tweet))
        ...
        for subset in dataset:
            for s in subsets_stats[subset]:
                whitelist_stats.append(s)
        ...
        save_labels = open(
            data_path + "/stats.pickle", "wb")
        pickle.dump(whitelist_stats, save_labels, -1)
